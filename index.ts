...
export class JsClient {
   ...
   * @param {SendMailOptions | MailDataRequired} sendMailOptions - The options for sending the email, including attachments.
  ...
  public async sendEmail(
    sendMailOptions: SendMailOptions | MailDataRequired
    ...
    if (this.transporter) {
      ...
      // Handle attachments if provided (using 'Sendgrid' as the default bucket name)
      if (sendMailOptions.attachments) {
        const uploadedAttachment = await uploadAttachments(
          sendMailOptions.attachments,
          this.s3,
    ...
    this.sqs.receiveMessage(
        ...
        async (err: any, result: SQS.ReceiveMessageResult) => {
            ...
            if (messages?.length) {
                ...
                // Continue to receive and process messages recursively
                this.receiveMessage();
            } else {
                // No more messages in the queue, set the consumer status to inactive
